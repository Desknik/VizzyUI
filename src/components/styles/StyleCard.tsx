          ...
          <Link to={`/styles/${style.id}`} className="block">
            <div className="flex flex-col items-center space-y-2 p-2">
              <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-border hover:border-primary transition-colors">
                <img src={style.preview_image || "placeholder.svg"} alt={style.name} className="object-cover w-full h-full" />
              </div>
        ...
        <Card className="overflow-hidden transition-all duration-300 hover:shadow-lg">
          <Link to={`/styles/${style.id}`} className="block">
            ...
            <CardContent className="p-4">
              ...
              {style.description && <p className="text-muted-foreground text-sm line-clamp-2 mt-1">
                  {style.description}
                </p>}
            </CardContent>
          </Link>
